...
public class SmartAnalytics
{
	...
	/// Report an event
	/// </summary>
	public static void SendEvent( string eventLabel, string value, string eventAction, string eventCategory )
	{
		string requestString = k_apiEndpoint + GetEventString()
			+ "&ec=" + WWW.EscapeURL( eventCategory ) + "&ea=" + WWW.EscapeURL( eventAction ) + ( ( eventLabel.Length > 0 ) ? ( "&el=" + WWW.EscapeURL( eventLabel ) ) : "" ) + "&ev=" + value
