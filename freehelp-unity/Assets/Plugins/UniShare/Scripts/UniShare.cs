...
public class UniShare: MonoBehaviour  {
	...
	#if UNITY_IPHONE
	//[DllImport ("__Internal")] private static extern void presentActivitySheetWithImageAndString(string message,byte[] imgData,int _length);

	//[DllImport ("__Internal")]	private static extern void pluginInit(string vkid);
	#endif

    private static UniShare s_instance = null;
    public static UniShare Instance
    {
        get
        {
            return s_instance;
	...
	{
        ShareNativeImage ( ShareText.Replace("<br>", "\n") );
	}
